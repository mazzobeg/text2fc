...
def main():
    ...
    client = openai.Client(
        organization=organization, project=project, api_key=OPEN_AI_KEY
    )
    timestamp = (
    ...
    if not USE_EXISTING_CREATED:
        ...
        for i, chunk in enumerate(chunks):
            ...
            messages = [
                {
                    "role": "system",
                    "content": open(
                        "./system-prompt-creator.txt", "r", encoding="utf-8"
                    )
                    .read()
                    .encode("utf-8")
                    .decode(),
            ...
            # Print the filename
            log.debug(
                f"[{(i+1)}/{len(chunks)}] Write chat gpt answer in file {filename}."
            )
